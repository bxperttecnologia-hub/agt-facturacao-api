'use strict';

// Testa POST /api/faturas/estado. Só faz sentido se o script 03 tiver
// recebido um requestID real da AGT (HTTP 200) — com credenciais de teste
// falsas, o 03 normalmente dá 502 e não há requestID nenhum para consultar,
// por isso este script avisa em vez de falhar nesse caso.
// Uso: node 04-consultar-estado-fatura.js [requestID]

const { chamar, mostrarResposta, lerEstado, credenciaisTeste } = require('./config');

(async () => {
    const requestID = process.argv[2] || lerEstado().request_id;
    if (!requestID) {
        console.log('AVISO: sem requestID guardado (o script 03 provavelmente não recebeu um da AGT). Nada a consultar.');
        return;
    }

    const corpo = { tax_id: "5002455595",
        private_key: `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`,
        nome_empresa: "Empresa de Teste, Lda",
        test: true, requestID };

    console.log('== POST /api/faturas/estado ==');
    const resposta = await chamar('POST', '/api/faturas/estado', corpo);
    mostrarResposta(resposta);

    if (resposta.status === 200) {
        console.log('OK: estado consultado com sucesso na AGT.');
    } else if (resposta.status === 502) {
        console.log('AVISO: a API respondeu correctamente, mas a AGT não respondeu à consulta de estado.');
    } else {
        console.error(`FALHOU: código HTTP inesperado (${resposta.status}).`);
        process.exit(1);
    }
})();
