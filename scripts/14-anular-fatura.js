"use strict";

const { chamar, mostrarResposta, guardarEstado } = require("./config");

// Série enviada pelo cliente
// const DOCUMENTO_A_ANULAR = process.argv[2];

// if (!DOCUMENTO_A_ANULAR) {
//   console.error("Uso: node 14-anular-fatura.js <NO da Fatura>");

//   process.exit(1);
// }

(async () => {
  const docNo = `FT FT9526S9096N/7`;
  const hoje = new Date().toISOString().split("T")[0];

  const corpo = {
    tax_id: "5002455595",
    private_key: `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`,
    nome_empresa: "Empresa de Teste, Lda",
    test: true,
    document_type: "FT",
    document_no: "FT FT9526S9096N/7",
    document_date: hoje,
    customer_tax_id: "999999999",
    customer_name: "Consumidor Final",
    customer_country: "AO",
    net_total: 1000,
    tax_payable: 140,
    gross_total: 1140,
    rejected_document_no: "FT FT9526S9096N/7",
    document_cancel_reason: "I",
    document_status: "A",
    lines: [
      {
        line_number: 1,
        product_code: "SERV001",
        product_description: "Serviço de teste",
        quantity: 1,
        unit_of_measure: "UN",
        unit_price: 1000,
        unit_price_base: 1000,
        debit_amount: 0,
        credit_amount: 1000,
        tax_type: "IVA",
        tax_country_region: "AO",
        tax_code: "NOR",
        tax_percentage: 14,
        tax_contribution: 140,
        settlement_amount: 0,
      },
    ],

    /**
     * Retenção na fonte
     *
     * Caso exista:
     *
     * [
     *   {
     *    type:"IRT",
     *    amount:16.5
     *   }
     * ]
     *
     */
    withholding_tax_list: [],
  };

  console.log("--------------------------------");
  console.log("POST /api/faturas");
  console.log("--------------------------------");

  console.log("Documento:", "FT FT9526S9096N/7");

  /**
   * Chama o proxy
   */
  const resposta = await chamar("POST", "/api/faturas", corpo);

  /**
   * Mostra resposta
   */
  mostrarResposta(resposta);

  /**
   * Guarda requestID devolvido pela AGT
   */
  const requestId = resposta.body?.resultado?.requestId;

  if (requestId) {
    guardarEstado("request_id", requestId);

    console.log("Request ID guardado:", requestId);
  }

  /**
   * Tratamento de respostas
   */
  if (resposta.status === 200) {
    console.log(`OK: Factura anulada aceite. RequestID=${requestId}`);
  } else if (resposta.status === 502) {
    console.log(
      "AVISO: O proxy funcionou, mas a AGT recusou ou não respondeu.",
    );
  } else {
    console.error(`ERRO: HTTP ${resposta.status}`);

    process.exit(1);
  }
})();
